...
| ② 建模 | 参数化 `.scad`（统一 `device(theta, pull, water_z, step, explode, section)` 接口）+ 流形 STL |
...
├── scripts/            14 个脚本（提取、渲染、分镜、配音、成片、校验）
...
`regression.yml` 每次 push 重跑它（含空白帧检测）；`full-pipeline.yml` 按需/每周跑一次带配音的完整成片。
